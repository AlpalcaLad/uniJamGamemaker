...
if !attacking{
	switch sprite_index{
		case archerWalkLower_s:
			spriteUpper=archer2WalkUpper_s
			imageUpper=image_index
		break;
		case archerIdleLower_s:
			spriteUpper=archer2IdleUpper_s
			imageUpper=image_index
		break;
		case archerInAirLower_s:
			spriteUpper=archer2InAirUpper_s
			imageUpper=image_index
		break;
	}
} else {
	...
	if !knifeThrown and imageUpper>4{
		knifeThrown=true
		//create knife
		var knife = instance_create_layer(x+sign(image_xscale)*4,y,layer,thrownMagicKnife_o)
		knife.target=target
